...
#define MAX_PACKET_SIZE 9  // Maximum possible UDP packet size (including headers)
#define PAYLOAD_SIZE 200     // Ideal payload size for UDP packets (leaving space for headers)

const int EXPIRY_DAY = 1;
const int EXPIRY_MONTH = 5;
const int EXPIRY_YEAR = 2025;

const int DEFAULT_THREAD_COUNT = 2;  // Set to 2 for 2-core system
;
std::mutex log_mutex;
...
int main(int argc, char *argv[]) {
    ...
    if (is_binary_expired()) {
        std::cerr << "ERROR: THIS BINARY HAS EXPIRED. PLEASE CONTACT THE DEVELOPER@Roxz_gaming." << std::endl;
        return EXIT_FAILURE;
    }
